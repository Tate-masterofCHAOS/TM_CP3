...
struct party{
    ...
    int player_turn(Mon opponent, party party){
    ...
    int battle_mon(Mon opponent, party party){
        cout << "A battle has started between " << mona.name << " and " << monb.name << "!" << endl;
        while (mona.health > 0 && opponent.health > 0 && !feld) {
            if (mona.SPD >= opponent.SPD) {
                player_turn(opponent, party);
...
int main() {
    Mon mon0 = {"Larose",5, 5, 5, 5, "", "Normal", "Fae", "Youre adding descriptions to them?", "Sarcasm", "Lightsaber", "Bad grade", "Tangent", 5, 5, 999, 0};
    Mon mon1 = {"Wanderer", 5,5, 5, 5, "The lost man mon", "Spirit", "Light", "After spending most of their life in haunted areas has infused them with many curses and possesions but unlike many spirit types they are uneffected to light and will use flashlights to blind it's prey before attacking", "Splish", "Splash", "Sploosh", "Splesh", 5, 5, 5, 5};
    Mon mon2 = {"George", 5,5, 5, 5, "The tickler mon", "Spirit", "Humor", "While slow, they get stronger the more damage they have done. To stun their prey they will tickle...that's it", "Tickle", "Clang", "Deciever deciever I shall cut thy pants with a cleaver", "", 5, 5, 5, 5};
    Mon mon3 = {"Flatline", 5,5, 5, 5, "The two dimensional mon", "Spirit", "Humor", "Ever wonder who is at fault, it is this mon's fault, they are reprehensible", "Blame", "Throw", "Slide", "Slash", 5, 5, 5, 5};
    Mon mon4 = {"Random fae in a bottle", 5,5, 5, 5, "The name stealer mon", "Fae", "Spirit", "Dave. There are many daves in the world, it is the fault of this mon, with a nasty habit of stealing and replacing names of people they meet they have many names but everyone they meet only has one, Dave.", "Davifiy", "Steal", "Float", "Screech", 5, 5, 5, 5};
    Mon mon5 = {"Abraham", 5, 5,5, 5, "The presidential mon", "Political", "Spirit", "Anybody who has been within the white house would know it is unnaturally cold, the reason for that is this mon always watching over his successors and making sure the ones he doesnt like dont get re-elected", "Mine", "Moon", "Men", "man", 5, 5, 5, 5};
    Mon mon6 = {"Chester", 5,5, 5, 5, "The strongest climbing mon", "Climber", "Swole", "The only living mon left, it spends eternity climbing random ledges", "climb", "scuttle", "jump", "hook", 5, 5, 5, 5};
    Mon mon7 = {"Togore", 5, 5,5, 5, "The golfer mon", "Golfer", "Stupoid", "Plays golf all day everyday which is stupid, golf is stupid", "hole in one", "birdie", "hole in three", "hole in two", 5, 5, 5, 5};
    ...
    while (true) {
        ...
        if (choice == "1") {
            ...
            Mon opponent = mon5; //wild encounter function here
